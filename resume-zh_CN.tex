...
\section{个人总结}
我有扎实的编程基础，熟悉python和c++，以及能够熟练使用codex和claude code等coding agent，此外我也具备丰富的linux使用(折腾)经验，以及一定的openclaw使用经验。我对多模态，计算机视觉，具身智能等领域有浓厚兴趣，曾进行过相关课程的自学和多个项目实践，有不错的自学和动手实操能力。我目前的未来规划是出国深造，但是最终我还是希望进入工业界工作
...
核心是让动作策略模型忽略相机观测中机械臂部分，避免模型过拟合到机械臂上，但是效果不好。研究中使用了SAM2、SAM3和ProPainter等项目，并对SAM2进行了微调，以及对\href{https://github.com/1JI0O/RISE-2/}{原始动作策略模型}的ViT等部分进行了修改

\vspace{1.0ex}

\textbf{软院的很多大作业}

例如QLink和QBasic : 基于Qt的连连看游戏和gui Basic解释器。其中QLink我分别用c++和python实现了两版。这两个项目我都开源放在github上了。
